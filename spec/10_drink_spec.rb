...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      subject(:fav_drink) { described_class.new('soda', 16)}
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(fav_drink.type).to be("soda")
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(fav_drink.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      subject(:fav_drink) {described_class.new('soda', 7)}
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type. In addition, specify ounces to be any number under 16.

      # remove the 'x' before running this test
      it 'is not full' do
        expect(fav_drink).to_not be_full
      end
    end
  end
end
